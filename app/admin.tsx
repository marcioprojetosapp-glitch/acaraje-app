...
export default function Admin() {
  ...
  function montarResumoDetalhado(pedido: any) {
        ...
        .map((i: any) => {
          ...
          if (Array.isArray(i.adicionais) && i.adicionais.length) {
            i.adicionais.forEach((c: any) => {
              const t = typeof c === "string" ? c : c.nome || c.nomeComplemento || c.title || c.label || "";
              if (t && String(t).trim()) extrasSet.add(String(t).trim().toUpperCase());
            });
          }
          if (i.obs && typeof i.obs === "string" && i.obs.trim() !== "") {
            i.obs.split(",").map((s: string) => s.trim()).filter(Boolean).forEach((p: string) => {
    ...
    if (pedido.resumoDetalhado && pedido.resumoDetalhado.length > 5) return pedido.resumoDetalhado;
  ...
  function imprimirPedidoTermica(pedido: any) {
    ...
    let iframe = document.getElementById("iframe-impressao") as HTMLIFrameElement;
    if (!iframe) {
    ...
    const html = `<html><head><title>${idCurto}</title><style>@page { size: 80mm auto; margin: 0; } body { width: 72mm; font-family: 'Courier New', monospace; font-size: 13px; padding: 4mm; margin:0; color:#000; } .titulo { text-align:center; font-weight:900; font-size:18px; } .sub { text-align:center; font-size:11px; } .linha { border-top:1px dashed #000; margin:8px 0; } .linha2 { border-top:2px solid #000; margin:8px 0; } .corte { page-break-after: always; } .big { font-size:16px; font-weight:900; }</style></head><body><div class="titulo">COZINHA - FRITAR</div><div class="sub">${data} | #${idCurto}</div><div class="linha2"></div><div class="big">${nomeCliente.toUpperCase()}</div><div class="linha"></div><div style="white-space:pre-wrap; font-size:15px; font-weight:bold; line-height:20px;">${resumo}</div><div class="linha2"></div><div style="text-align:center; font-weight:900; font-size:14px;">*** COZINHA ***</div><br/><br/><br/><div class="corte"></div><div class="titulo">ACARAJE DA BENCAO</div><div class="sub">VIA ENTREGA | #${idCurto}<br/>${data}</div><div class="linha2"></div><div><b>CLIENTE:</b> ${nomeCliente}</div><div><b>ZAP:</b> ${pedido.whatsapp || pedido.telefone || ""}</div><div><b>END:</b> ${pedido.endereco || "RETIRADA NO BALCAO"}</div><div><b>PAG:</b> ${(pedido.formaPagamento || "").toUpperCase()} - PAGO</div><div class="linha"></div><div style="white-space:pre-wrap; font-size:13px; line-height:18px;">${resumo}</div><div class="linha"></div><div style="display:flex; justify-content:space-between; font-size:18px; font-weight:900;"><span>TOTAL</span><span>R$ ${totalExibir}</span></div><div class="linha2"></div><div style="text-align:center; font-weight:900;">*** MOTOBOY ***</div><script>window.onload = function(){ window.print(); }</script></body></html>`;
    const docIframe = iframe.contentDocument || iframe.contentWindow?.document;
    if (docIframe) { docIframe.open(); docIframe.write(html); docIframe.close(); }
  }

  useEffect(() => {
    if (!autorizado) return;
    return onSnapshot(query(collection(db, "pedidos"), orderBy("criadoEm", "desc")), async (s) => {
  ...
  useEffect(() => {
    if (!autorizado) return;
    return onSnapshot(collection(db, "produtos"), (s) => setProdutos(s.docs.map((d) => ({ id: d.id, ...d.data() }))));
  }, [autorizado]);
  ...
  const apagarPedido = async (id: string) => {
    try {
      const confirma = Platform.OS === "web" ? window.confirm("Apagar esse pedido?") : true;
      if (Platform.OS !== "web") {
        Alert.alert("Apagar?", "Apagar esse pedido?", [
          { text: "Cancelar", style: "cancel" },
          { text: "Apagar", style: "destructive", onPress: async () => { await deleteDoc(doc(db, "pedidos", id)); } },
        ]);
        return;
      }
      if (!confirma) return;
  ...
  const apagarTodos = async () => {
    try {
      const confirma = Platform.OS === "web" ? window.confirm("APAGAR TODO HISTÓRICO DE TESTES?") : true;
      if (Platform.OS !== "web") {
    ...
    } catch (e: any) { alert("Erro ao apagar tudo: " + e.message); }
  };
  ...
  const uploadToCloudinary = async (uri: string) => {
    ...
    try {
      ...
      if (data.secure_url) {
        if (editando) setEditando({ ...editando, imagemURL: data.secure_url } as any);
        else setImageUrl(data.secure_url as any);
      }
    } finally { setUploading(false); }
  };
  const pickImage = async () => {
    let r = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsEditing: true, aspect: [1, 1], quality: 0.7 });
  ...
  const salvarProduto = async () => {
    ...
    await addDoc(collection(db, "produtos"), { nome, preco: parseFloat(preco.replace(",", ".")), descricao, estoque: qtd, imagemURL: imageUrl, disponivel: qtd > 0, criadoEm: serverTimestamp() });
  ...
  const marcarPago = async (p: any) => { await updateDoc(doc(db, "pedidos", p.id), { pago: true, status: "pago" }); };
  const marcarEntregue = async (p: any) => { await updateDoc(doc(db, "pedidos", p.id), { status: "entregue" }); };
    ...
    <View style={{ flex: 1, backgroundColor: "#000" }}>
      <Modal visible={modalSenhaVisivel} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.95)", justifyContent: "center", alignItems: "center", padding: 20 }}>
          <View style={{ width: "85%", backgroundColor: "#1a1a1a", padding: 22, borderRadius: 14, borderWidth: 1, borderColor: "#D4AF37" }}>
            <Text style={{ color: "#D4AF37", fontWeight: "900", fontSize: 18, textAlign: "center" }}>ADMIN ACARAJÉ DA BENÇÃO</Text>
            <TextInput style={{ borderWidth: 1, borderColor: "#333", backgroundColor: "#000", color: "#fff", padding: 14, borderRadius: 10, textAlign: "center", fontSize: 20, marginTop: 15 }} placeholder="Digite a senha" placeholderTextColor="#666" value={senhaDigitada} onChangeText={setSenhaDigitada} secureTextEntry={true} keyboardType="number-pad" autoFocus autoComplete="off" />
            <TouchableOpacity onPress={verificarSenha} style={{ backgroundColor: "#D4AF37", padding: 14, borderRadius: 10, marginTop: 15, alignItems: "center" }}><Text style={{ fontWeight: "900" }}>ENTRAR</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
      {autorizado && (
        <ScrollView style={{ flex: 1, backgroundColor: "#000", padding: 12, paddingTop: 45 }}>
          ...
          {aba === "pedidos" && (
            <View>
              <View style={{ flexDirection: "row", gap: 6, marginBottom: 12 }}>
                {[{ id: "todos", lb: "TODOS" },{ id: "novo", lb: "A PAGAR" },{ id: "pago", lb: "PAGOS" },{ id: "entregue", lb: "ENTREGUES" }].map((f) => (
                  <TouchableOpacity key={f.id} onPress={() => setFiltro(f.id)} style={{ flex: 1, padding: 9, borderRadius: 20, backgroundColor: filtro === f.id ? "#D4AF37" : "#222", alignItems: "center" }}>
                    <Text style={{ fontSize: 10, fontWeight: "900", color: filtro === f.id ? "#000" : "#fff" }}>{f.lb}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <View style={{ gap: 12 }}>
                {pedidosFiltrados.map((p: any) => {
                  ...
                  return (
                    <View key={p.id} style={{ backgroundColor: isEntregue ? "#111" : isPagoAuto ? "#112911" : "#1a1a1a", padding: 14, borderRadius: 14, borderWidth: 1, borderColor: isEntregue ? "#333" : isPagoAuto ? "#00C851" : "#444" }}>
                      ...
                      <Text style={{ color: "#fff", fontWeight: "900", fontSize: 16, marginTop: 8 }}>{p.nome || "Cliente"} • R$ {p.total}</Text>
                      <Text style={{ color: "#ddd", marginTop: 6, fontSize: 13, lineHeight: 19, backgroundColor: "#000", padding: 8, borderRadius: 8 }}>{montarResumoDetalhado(p)}</Text>
                      <View style={{ marginTop: 8, gap: 3 }}>
                        <Text style={{ color: "#aaa", fontSize: 12 }}>{isEntrega ? `📍 ${p.endereco}` : "📍 Retira na loja"}</Text>
          ...
          {aba === "produtos" && (
            <View>
              <View style={{ backgroundColor: "#1a1a1a", padding: 14, borderRadius: 12, borderWidth: 1, borderColor: "#333" }}>
                <Text style={{ color: "#D4AF37", fontWeight: "900", marginBottom: 10 }}>CADASTRAR PRODUTO</Text>
                <TouchableOpacity onPress={pickImage} style={{ backgroundColor: "#222", height: 120, borderRadius: 10, justifyContent: "center", alignItems: "center", marginBottom: 10, borderWidth: 1, borderColor: "#333" }}>
                  {imageUrl ? <Image source={{ uri: imageUrl as any }} style={{ width: "100%", height: "100%", borderRadius: 10 }} /> : <Text style={{ color: "#888" }}>{uploading ? "ENVIANDO..." : "📷 FOTO"}</Text>}
                </TouchableOpacity>
                <TextInput placeholder="Nome" placeholderTextColor="#666" value={nome} onChangeText={setNome} style={{ backgroundColor: "#000", color: "#fff", padding: 12, borderRadius: 8, borderWidth: 1, borderColor: "#333", marginBottom: 8 }} />
                <TextInput placeholder="Preço ex: 15.00" placeholderTextColor="#666" value={preco} onChangeText={setPreco} keyboardType="numeric" style={{ backgroundColor: "#000", color: "#fff", padding: 12, borderRadius: 8, borderWidth: 1, borderColor: "#333", marginBottom: 8 }} />
                <TextInput placeholder="Descrição" placeholderTextColor="#666" value={descricao} onChangeText={setDescricao} style={{ backgroundColor: "#000", color: "#fff", padding: 12, borderRadius: 8, borderWidth: 1, borderColor: "#333", marginBottom: 8 }} />
                <TextInput placeholder="Estoque" placeholderTextColor="#666" value={estoque} onChangeText={setEstoque} keyboardType="numeric" style={{ backgroundColor: "#000", color: "#fff", padding: 12, borderRadius: 8, borderWidth: 1, borderColor: "#333", marginBottom: 10 }} />
                <TouchableOpacity onPress={salvarProduto} style={{ backgroundColor: "#D4AF37", padding: 14, borderRadius: 10, alignItems: "center" }}><Text style={{ fontWeight: "900" }}>SALVAR PRODUTO</Text></TouchableOpacity>
              </View>
              <View style={{ gap: 10, marginTop: 15 }}>
                {produtos.map((pr: any) => (
                  <View key={pr.id} style={{ backgroundColor: "#1a1a1a", padding: 12, borderRadius: 10, flexDirection: "row", alignItems: "center", gap: 10, borderWidth: 1, borderColor: "#333" }}>
                    <Image source={{ uri: pr.imagemURL }} style={{ width: 50, height: 50, borderRadius: 8, backgroundColor: "#222" }} />
                    <View style={{ flex: 1 }}>
                      <Text style={{ color: "#fff", fontWeight: "900" }}>{pr.nome}</Text>
                      <Text style={{ color: "#D4AF37" }}>R$ {pr.preco}</Text>
                      <Text style={{ color: pr.disponivel ? "#00C851" : "red", fontSize: 10 }}>{pr.disponivel ? `ESTOQUE: ${pr.estoque}` : "SEM ESTOQUE"}</Text>
                    </View>
                    <TouchableOpacity onPress={() => { setEditando(pr); setEditModal(true); }} style={{ backgroundColor: "#333", padding: 10, borderRadius: 8 }}><Text style={{ color: "#fff", fontSize: 10 }}>EDITAR</Text></TouchableOpacity>
                    <TouchableOpacity onPress={async () => { if (Platform.OS === "web" ? window.confirm("Apagar produto?") : true) await deleteDoc(doc(db, "produtos", pr.id)); }} style={{ backgroundColor: "#330000", padding: 10, borderRadius: 8 }}><Text style={{ color: "red", fontSize: 10 }}>X</Text></TouchableOpacity>
                  </View>
                ))}
              </View>
            </View>
          ...
          {aba === "config" && (
            <View style={{ backgroundColor: "#1a1a1a", padding: 16, borderRadius: 12, borderWidth: 1, borderColor: "#333" }}>
              ...
              <Text style={{ color: "#888", fontSize: 12, marginBottom: 4 }}>Taxa de Entrega (R$)</Text>
              <TextInput value={String(config.taxaEntrega)} onChangeText={(t) => setConfig({ ...config, taxaEntrega: Number(t) || 0 })} keyboardType="numeric" style={{ backgroundColor: "#000", color: "#fff", padding: 12, borderRadius: 8, borderWidth: 1, borderColor: "#333", marginBottom: 12 }} />
              <Text style={{ color: "#888", fontSize: 12, marginBottom: 4 }}>Tempo Médio</Text>
              <TextInput value={config.tempoMedio} onChangeText={(t) => setConfig({ ...config, tempoMedio: t })} style={{ backgroundColor: "#000", color: "#fff", padding: 12, borderRadius: 8, borderWidth: 1, borderColor: "#333", marginBottom: 12 }} />
              <TouchableOpacity onPress={salvarConfig} style={{ backgroundColor: "#D4AF37", padding: 14, borderRadius: 10, alignItems: "center", marginTop: 10 }}><Text style={{ fontWeight: "900" }}>SALVAR CONFIG</Text></TouchableOpacity>
            </View>
          )}
        </ScrollView>
      )}
